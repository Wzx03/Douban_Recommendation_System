...
# # 🌟 新增：剔除演唱会、音乐剧等非纯粹电影的条目
# movies_df = movies_df[~movies_df['类型'].astype(str).str.contains('音乐|脱口秀|戏曲', na=False)]
...
print(f"初步处理完毕：保留了 {len(users_df)} 条打分记录。")

# 5. 提取黄金三元组
ratings_model_data = users_df[['用户ID', 'movie_id', '评分']]

print("\n========== 核心算法输入数据 (黄金三元组) ==========")
print(f"清洗前原始打分矩阵大小: {ratings_model_data.shape[0]} 行")

# # 🌟 新增大招：降噪清洗（剔除长尾数据），极大降低 RMSE
# print("🧹 正在执行降噪清洗：剔除冷门用户和冷门电影...")
#
# # 剔除评价次数少于 5 次的用户
# user_counts = ratings_model_data['用户ID'].value_counts()
# active_users = user_counts[user_counts >= 5].index
# ratings_model_data = ratings_model_data[ratings_model_data['用户ID'].isin(active_users)]
#
# # 剔除被评价少于 5 次的电影
# movie_counts = ratings_model_data['movie_id'].value_counts()
# popular_movies = movie_counts[movie_counts >= 5].index
# ratings_model_data = ratings_model_data[ratings_model_data['movie_id'].isin(popular_movies)]

print(f"✨ 清洗后(已过滤冷门噪声)有效高密度矩阵大小: {ratings_model_data.shape[0]} 行")
